...
if ! [ -x "$(command -v psql)" ]; then
  ...
  echo >&2 "Install on MacOS using Homebrew:"
  echo >&2 "   brew doctor"
  echo >&2 "   brew update"
  echo >&2 "   brew install libpq"
  echo >&2 "   brew link --force libpq"
...
#Check if a custom database name gas been set, otherwise default to 'newsletter'
